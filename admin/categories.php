    ...
    if (!hash_equals($_SESSION['csrf'] ?? '', $_POST['csrf'] ?? '')) {
        ...
        try {
            if ($act === 'create') {
                ...
                if ($sportId <= 0 || $name === '') throw new RuntimeException('ข้อมูลไม่ครบ');
                $panel->createCategory($sportId, $name);
                $ok = 'สร้างแล้ว';
            } elseif ($act === 'update') {
                $id = (int)($_POST['id'] ?? 0);
                $sportId = (int)($_POST['sport_id'] ?? 0);
                $name = trim($_POST['name'] ?? '');
                if ($id <= 0 || $sportId <= 0 || $name === '') throw new RuntimeException('ข้อมูลไม่ครบ');
                $panel->updateCategory($id, $sportId, $name);
                $ok = 'อัปเดตแล้ว';
            } elseif ($act === 'delete') {
                $id = (int)($_POST['id'] ?? 0);
                if ($id <= 0) throw new RuntimeException('bad id');
                $panel->deleteCategory($id);
                $ok = 'ลบแล้ว';
            }
        } catch (Throwable $e) {
...
<head>
    ...
    <title>Categories — YRScores</title>
...
<body class="bg-white">
    <div class="max-w-5xl mx-auto p-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold">หมวด/ลีก (Categories)</h1>
        ...
        <form method="post" class="border rounded-2xl p-4 mb-6">
            ...
            <div class="grid sm:grid-cols-3 gap-3">
                ...
                <input name="name" placeholder="เช่น Premier League, NBA" class="border rounded p-2" required>
                <button class="rounded text-white px-4" style="background:#FF1493">เพิ่มหมวด/ลีก</button>
            </div>
        </form>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="text-left p-2">ชื่อหมวด/ลีก</th>
                ...
                <tbody>
                        ...
                        <tr class="border-t">
                            ...
                            <td class="p-2">
                                <form method="post" class="flex flex-wrap gap-2 items-center">
                                    ...
                                    <select name="sport_id" class="border rounded p-1">
                                        <?= sportOptions($sports) ?>
                                    </select>
                                    ...
                                    <button name="action" value="delete" class="px-3 py-1 rounded border border-red-500 text-red-600" onclick="return confirm('ลบหมวด/ลีกนี้?')">ลบ</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach;
                    if (!$rows): ?>
                        <tr>
                            <td colspan="5" class="p-3 text-gray-500">ยังไม่มีรายการ</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
